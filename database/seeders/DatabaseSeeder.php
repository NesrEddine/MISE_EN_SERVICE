    ...
    public function run(): void
    {
        ...
        $Services = $this->withProgressBar(50, fn () => Service::factory(1)
            ...
            ->has(
                Comment::factory()->count(rand(10, 20))
                    ->state(fn (array $attributes, Service $product) => ['customer_id' => $customers->random(1)->first()->id]),
            )
            ->create());
        ...
        $orders = $this->withProgressBar(1000, fn () => Order::factory(1)
            ...
            ->has(
                OrderItem::factory()->count(rand(2, 5))
                    ->state(fn (array $attributes, Order $order) => ['shop_product_id' => $Services->random(1)->first()->id]),
                'items'
            )
            ->create());
        ...
        foreach ($orders->random(rand(5, 8)) as $order) {
            Notification::make()
                ...
                ->body("{$order->customer->name} ordered {$order->items->count()} Services.")
                ->actions([
                    Action::make('View')
